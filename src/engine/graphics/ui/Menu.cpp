    ...
    if (hover_) {
        ...
        if (!children_.empty()) {
            ...
            if (parent_menu_.expired()) {
                ...
                sub_menu_panel_->setOrigin(origin_ + + glm::vec2{size_.x + 2, 0});
                mdd.viewport = {origin_.x + size_.x +2, origin_.y, size_.x * 4, panelSizeY };
            }
...
MessageHandleResult Menu::onMessage(const UIMessage &message) {
    switch (message.type) {
        case MessageType::MouseMove: {
            ...
            if (sub_menu_open) {
                hover_sub_menu_panel = Widget::checkMouseOver(message.mouseMoveMessage.x, message.mouseMoveMessage.y, sub_menu_panel_.get());
            }
            hover_ = hover_menu;
